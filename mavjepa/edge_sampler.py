...
class EdgeSampler:
    def __init__(self, mode: str = "none", edge_budget: int = 1, p_min: float = 0.05, seed: int = 0):
    ...
    def probabilities(self, edges: list[dict[str, Any]]) -> dict[str, float]:
        ...
        for edge in edges:
            ...
            quality = float(edge.get("quality", 1.0))
            score = math.sqrt(max(loss, 0.0) + 1e-8) * quality
    ...
    def update_loss(self, edge_name: str, loss_value: float, beta: float = 0.95) -> None:
        current = self.ema_loss.get(edge_name, loss_value)
        self.ema_loss[edge_name] = beta * current + (1.0 - beta) * loss_value
